...
if (!class_exists('FAPIClient')) {
	...
	class FAPIClient {
		...
		public function __construct($username, $password, $url = 'http://api.fapi.cz') {
			...
			$resources = array('client', 'invoice', 'item', 'periodicInvoice', 'currency', 'paymentType', 'country', 'settings', 'email', 'log', 'user', 'itemTemplate');
	...
	class FAPIClient_InvoiceResource extends FAPIClient_Resource {
		...
		public function getPdf($id)
		{
			try {
				...
				$this->setCode();
				return $this->client->last_response['body'];
			} catch (FAPIClient_Pest_Exception $exception) {
				$this->parent->processException($exception);
			}
		}
		...
		{
			try {
				$response = $this->client->post($this->url . '/send-email', array(
					'invoice' => $id,
	...
	class FAPIClient_StatisticsResource extends FAPIClient_Resource {
		...
		{
			try {
				$response = $this->client->post($this->url . '/client-totals', array(
					'clients' => $clientIds,
				));
				$this->setCode();
				return $response;
