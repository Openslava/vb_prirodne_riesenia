...
if(jQuery) (function($) {
	...
	function init(input, settings) {
		...
		// The input
    var used_colors='<div class="used_colors">';
    if (ve_used_colors.length > 0){
        $.each(ve_used_colors , function( key, value ) {
            used_colors=used_colors+'<span data-color="'+value+'" style="background: '+value+';" title="'+value+'"></span>';
        });
    }
    used_colors+='</div>';
		input
    ...
    .on('click.minicolors', '.used_colors span', function(event) {
        var color=$(this).attr('data-color');
        var input;
        input = $(this).closest('.minicolors').find('.minicolors-input');
        input.val(color);
        minicolors = input.parent();
  			
  			minicolors.find('.minicolors-swatch-color').css({
