...
$args=array(
	'prev_text' => file_get_contents(__DIR__.'/images/left.svg', true),
	'next_text' => file_get_contents(__DIR__.'/images/right.svg', true),
);
...
function cms_write_post($type=1, $cols=1, $article_class='', $num) {
    ...
    if($type==2 && ($post_format=='quote' || $post_format=='video')) $type=1;
    ...
    if($type==2) $thumb_size='blog_medium';
		...
		if(isset($blog_module->appearance['show_button'])) $article_class.=' article_wbutton';
    ...
    echo '<div id="article_'.$post->ID.'" class="article '.$article_class.' '.((!$thumb)?'article_nothumb':'').' article_type_'.$type.'">';  
		...
		if(isset($setting['show_share_list']['facebook'])) { 
					...
					$share_container='<div class="article_hover_container"><div class="fb-like" data-href="'.get_permalink($post->ID).'" data-layout="button_count" data-action="like" data-show-faces="false" data-share="'.$share.'"></div></div>';
		} else if(isset($setting['show_share_list']['facebook_share'])) {
					$share_container='<div class="article_hover_container"><div class="fb-share-button" data-href="'.get_permalink($post->ID).'" data-layout="button_count"></div></div>';
		} 
    ...
    //standard format    
    else if($thumb && $post_format!='quote') echo '<a href="'.get_permalink($post->ID).'" class="thumb">'.get_the_post_thumbnail($post->ID,$thumb_size).$share_container.'</a>';
    ...
    if ( $post_format=='quote'  ) {
        ...
        echo '<div class="article_body">';
				...
				if(!$thumb) echo $share_container;
				  
				if(!isset($setting['hide']['date'])) echo '<span class="date">'.get_the_date('j.n. Y').'</span>';
        echo '<h2><a href="'.get_permalink().'" rel="bookmark" title="'.get_the_title().'">'.get_the_title().'</a></h2>         
		...
		echo '<div class="article_footer">';
				...
				if(isset($blog_module->appearance['show_button'])) 
						echo '<div class="article_button_container"><a class="ve_content_button ve_content_button_1 article_button_more" href="'.get_permalink().'">'.__('Celý článek','cms_blog').'</a></div>';
				
				// article meta 
		    echo '<div class="article_meta">';
						echo '<a class="user" href="'.get_author_posts_url($post->post_author).'">'.get_avatar(get_the_author_meta( 'ID' ), 24 ).get_the_author().'</a>';
		        if(isset($setting['comments']['wordpress'])) echo '<a class="comments" href="'.get_comments_link().'">'.file_get_contents(__DIR__."/images/comments.svg").$comments.' <span></span></a>';
