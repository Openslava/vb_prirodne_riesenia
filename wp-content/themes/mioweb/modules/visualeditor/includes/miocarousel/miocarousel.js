...
;(function(factory){
  ...
  var MioCarousel = (function(element, settings){
    ...
    function _MioCarousel(element, settings){
      ...
      if($(element).is('[data-centered]')) centered=$(element).attr("data-centered");
      ...
      this.defaults = {
        ...
        animation: animation,
        arrowRight: '.mc_arrow_container-right span',
        arrowLeft: '.mc_arrow_container-left span',
        autoplay: autoplay,
      };
      ...
      if(centered) {
        this.$el.find('.'+centered).css({
            marginTop: ((this.$el.height() - this.$el.find('.'+centered).innerHeight())/2)
        });
      }
